...
def average_absolute_delta(img):
	gray_average = np.average(img)
	delta = img - gray_average
	return np.average(np.absolute(delta))
...
if __name__ == "__main__":
	...
	for path in gt_path:
		...
		if len(det_boxes_list) > 0:
			for gt_box in gt_boxes_list:
				...
				# Average of Delta Absolute
				aad_i = average_absolute_delta(cropped)
				...
				average_absolute_deltas.append(aad_i)
	...
	if graph_type == 0:
		...
		for i in range(len(edgeness_values)):
			data.append([edgeness_values[i], average_absolute_deltas[i], error[i]])
	
	else:
		for i in range(len(edgeness_values)):
			data.append([average_absolute_deltas[i], avg_grays[i], error[i]])
	...
	if graph_type == 0:
		...
		ax.set_xlabel('Edgeness', fontweight ='bold')
		ax.set_ylabel('Average of Delta Absolutes', fontweight ='bold')  
	
	else:
		ax.set_xlabel('Average of Delta Absolutes', fontweight ='bold')
		ax.set_ylabel('Average Gray', fontweight ='bold') 
